...
public class Backend {
    ...
    public List<String> getShippingDetails(List<String> orderIds) {
        String SHIP_SQL = "  select 'Incomm'||','||'1454123'||','||vod_parent_oid||','||vod_order_id||','||TO_CHAR(sysdate, 'MM/DD/YYYY')||','||cap_serial_number||','||'1'||','||vol_package_id||','||'GC '||''||','||'Incomm'||','||'Incomm'||','||'250 Williams Street Suite 500'||','||','||'Atlanta'||','||'GA'||','||'30303'||','||'42433112'||','||TO_CHAR(sysdate, 'MM/DD/YYYY')||','||'N/A'||','||VSM_SHIPMENT_ID||','||vod_shiPPING_METHOD as details\n" +
